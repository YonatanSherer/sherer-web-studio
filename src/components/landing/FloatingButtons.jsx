    ...
    <div className="relative flex items-center group">
      {hovered && (
        <div className={`absolute ${isRTL ? "left-14" : "right-14"} bg-[#060a14] text-white text-xs font-medium px-3 py-1.5 rounded-lg shadow-lg whitespace-nowrap border border-white/10 pointer-events-none z-10`}>
          {tooltip}
          <div className={`absolute top-1/2 ${isRTL ? "left-0 -translate-x-1" : "right-0 translate-x-1"} -translate-y-1/2 w-2 h-2 rotate-45 bg-[#060a14] ${isRTL ? "border-l border-b" : "border-r border-t"} border-white/10`} />
        </div>
      )}
      <a
        href={href}
  ...
  return (
    <>
      ...
      <div className={`hidden md:flex fixed bottom-7 z-50 flex-col gap-2 ${isRTL ? "left-7" : "right-7"}`}>
        {buttons.map((btn) => (
          <FloatBtn key={btn.label} {...btn} onClick={btn.track} isRTL={isRTL}>
            {btn.icon}
          </FloatBtn>
        ))}
      </div>
